...
export class CatalogService {
   ...
   * @throws {CatalogException} if catalog name already exists
   */
  async createCatalog(
    ...
    if (catalog) {
      this.logger.warn(
        `Catalog name ${createCatalogDto.name} does exists`,
        context,
      );
      throw new CatalogException(CatalogValidation.CATALOG_EXITS);
    }

    const catalogData = this.mapper.map(
      createCatalogDto,
      CreateCatalogRequestDto,
      Catalog,
    );
    ...
    const catalogDto = this.mapper.map(
      createdCatalog,
      Catalog,
      CatalogResponseDto,
    );
  ...
  async getAllCatalogs(): Promise<CatalogResponseDto[]> {
    ...
    const catalogsDto = this.mapper.mapArray(
      catalogs,
      Catalog,
      CatalogResponseDto,
    );
   ...
   * @throws {CatalogException} if the catalog with the specified slug is not found
   */
  async updateCatalog(
    slug: string,
    ...
    this.logger.log(`Catalog ${slug} updated successfully`, context);
    const catalogDto = this.mapper.map(
      updatedCatalog,
      Catalog,
      CatalogResponseDto,
   ...
   */
  async deleteCatalog(slug: string): Promise<number> {
    ...
    if (catalog.products?.length > 0) {
      this.logger.warn(
        `Must change catalog of products before delete catalog ${slug}`,
        context,
      );
      throw new CatalogException(CatalogValidation.DELETE_CATALOG_ERROR);
    }

    const deleted = await this.catalogRepository.softDelete({ slug });
    this.logger.log(`Catalog ${slug} deleted successfully`, context);
