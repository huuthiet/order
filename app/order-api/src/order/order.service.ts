...
export class OrderService {
  ...
  async createOrder(
    ...
    const checkValidOrderItemData = await this.checkDataCreateOrderItem(requestData.orderItems);
  ...
  async checkDataCreateOrder(
    ...
    Object.assign(order, {
      owner: owner,
      branch: branch,
      tableName: (data.type === OrderType.AT_TABLE ? checkTable.taleData.name: null),
    })
  ...
  async checkDataCreateOrderItem(
    data: CreateOrderItemRequestDto[]
  ): Promise<{ isValid: Boolean, mappedOrderItems?: OrderItem[], subtotal?: number}> {
